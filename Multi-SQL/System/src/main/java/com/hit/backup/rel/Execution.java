...
//            System.out.println("cassandra端口:" + assinged_port_cassa);
            ...
            this.cluster = Cluster.builder().addContactPoints("127.0.0.1").withPort(9042).build();
            this.cassandrasession = cluster.connect("user_demo");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
...
//            this.session.disconnect();
                        ...
                        for (String uid : ids) {
                            ...
                            if (resultSet2 != null) {
                                for (Row row : resultSet2) {
                                    if (result1.contains(new Integer(row.getInt(0)).toString())) {
                                        results.add(new Integer(row.getInt(0)).toString());
                                    }
                                }
                            }
                        }
                        ...
                        for (String id : ids) {
                            ...
                            if (resultSet1 != null) {
                                for (Row row : resultSet1) {
                                    result1.add(new Integer(row.getInt(0)).toString());
                                }
                            }
                        }
                        ...
                        for (String id : ids) {
                            ...
                            if (resultSet != null) {
                                for (Row row : resultSet) {
                                    results.add(row.getInt(0) + " " + row.getString(3) + " " +
                                            row.getInt(1) + " " + row.getString(2) + " " + row.getString(4)
        ...
        if (mode == 0) {
            try {
                ...
                if (sql.contains("select activity.device")) {
                    ...
                    String v = sql.substring(sql.indexOf("[") + 1, sql.indexOf("]"));
                    String[] uids = v.split(",");
                    if (uids.length != 0 && sql.indexOf("[") + 1 != sql.indexOf("]")) {
                        StringBuilder sb = new StringBuilder();
                        sb.append("uid = " + uids[0]);
                        int i = 1;
                        while (i < uids.length) {
                        ...
                        resultSet = st.executeQuery("select device, count(device) from activity " +
                                "where log_time between " + start + " and " + end + " group by device");
                    }
                } else if (sql.contains("Select rel.id")) {
                    //if(sql.contains(">="))

                    String register_date = sql.substring(sql.indexOf("<") + 2, sql.indexOf("<") + 10);
                    String log_time = sql.substring(sql.indexOf("<") + 26, sql.indexOf("<") + 34);
                    if (sql.contains(">")) {
                    ...
                    if (uids.length != 0 && sql.indexOf("[") + 1 != sql.indexOf("]")) {
                        ...
                        System.out.println("select * from basicinfo where register_date < " + register_date);
                        resultSet = st.executeQuery("select * from basicinfo where register_date < " + register_date);
                    }
                } else if (sql.equals("Select rel.id where rel.age = 20")) {
                    resultSet = st.executeQuery("select id from basicinfo where age = 20");
                }
                else if(sql.equals("Select rel.id where rel.age between 35 and 45")){
                    resultSet = st.executeQuery("select id from basicinfo where age >= 35 and age <= 45");
                    //System.out.println(resultSet);
                }
                else if(sql.equals("Select rel.id where rel.age between 20 and 30")){
                    resultSet = st.executeQuery("select id from basicinfo where age >= 20 and age <= 30");
                    //System.out.println(resultSet);
                }
                if (resultSet != null) {
                    int count = resultSet.getMetaData().getColumnCount();
    ...
    public String[] performanceTest() {
        ...
        try {
            Statement st = mysqlConnection.createStatement();
            st.execute("delete from activity where id > 25600000");
            st.close();
        ...
        try {
            ...
            try {
                ...
                while ((rec = br.readLine()) != null) {
                    String tmp2[] = rec.split(",");
                    int id = Integer.valueOf(tmp2[0]);
                    int uid = Integer.valueOf(tmp2[1]);
                    String date = tmp2[2];
                    String year = date.substring(0, 4);
                    String month = date.substring(4, 6);
                    String day = date.substring(6, 8);
            ...
            results[0] = "Path: user_inform.csv"+"\n"+
                    "Type: CSV"+"\n"+
                    "Count: 2560000"+"\n"+
                    "Size: 89MB"+"\n"+
    ...
    public String[] loadWorkload() {
        ...
        String results[] = new String[3];
        results[0] = "Path：workload_default.txt"+"\n"+
                "Read operation: 30"+"\n"+
                "write operation: 10"+"\n"+
                "type: read_only"+"\n"+
                "size: 4KB"+"\n"+
