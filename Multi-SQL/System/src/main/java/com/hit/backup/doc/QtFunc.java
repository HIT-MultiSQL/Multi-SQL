...
public class QtFunc {
	...
	private String src = "/home/wangyutong";
	private int dataNum = 1;
	private int workloadNum = 1;
	public List<String> uploadData(String path) {
		List<String> res = this.runInPython(src+"/python/data_load.py", new String[]{src+path,"data"+dataNum,this.dbname,this.collection});
		dataNum += 1;
...
//		ress.add(details);
//		ress.add(configuration);
		return res;
	}
	
	public List<String> uploadWorkload(String path) {
		List<String> res = this.runInPython(src+"/python/workload_load.py", new String[]{src+path,"workload"+workloadNum});
		workloadNum += 1;
		return res;
	}
	...
	public List<String> indexSelect(List<String> dataDetials, List<String> workloadDetials){
		...
		for(int i = 0;i < num;i++) {
			...
			List<String> r = this.runInPython(src+"/python/index_select.py", new String[]{datapath,workloadpath,document[0].trim(),document[1].trim(),src,this.dbname,this.collection});
